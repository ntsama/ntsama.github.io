---
layout: page
title: "Projet MSCA — Trapèze Cognitif"
permalink: /msca_fr/
lang: fr
---

<div style="text-align:right; font-size:0.9rem;">
  🌐 <a href="/msca/">FR</a> | <a href="/msca_en/">EN</a>
</div

# Projet MSCA  
## Cognitive Trapeze : Modéliser les dynamiques langage–pensée dans l’apprentissage augmenté par IA  

---

# Résumé Exécutif  
**Cognitive Trapeze** étudie la co‑évolution du **langage et de la pensée** dans des environnements d’apprentissage augmentés par IA.  
Le projet combine analyse conceptuelle, expérimentations cognitives, données multilingues et modélisation computationnelle.

Résultats attendus :  
- un modèle théorique unifié  
- des données empiriques sur les apprenants multilingues et neurodivergents  
- une simulation multi‑agents  
- des ressources éducatives ouvertes  
- un impact Europe–Afrique en éducation numérique  

---

# PARTIE B1 — EXCELLENCE  

## 1. Titre du Projet  
Cognitive Trapeze: Modeling Language–Thought Dynamics in AI‑Augmented Learning

## 2. Résumé  
Ce projet étudie comment **langage et pensée se co‑développent** dans des environnements d’apprentissage augmentés par l’IA.  
En s’appuyant sur le **Modèle du Trapèze‑Balançoire Pédagogique**, il propose un cadre dynamique pour modéliser la manière dont les apprenants naviguent entre structures linguistiques, représentations cognitives et médiation technologique.

## 3. Objectifs  

### Objectifs Scientifiques  
- Modéliser l’interaction dynamique entre **langage, pensée et technologie**  
- Analyser l’influence des outils d’IA sur le développement conceptuel  
- Étudier les profils multilingues et neurodivergents  

### Objectifs Méthodologiques  
- Intégrer approches théoriques, empiriques et computationnelles  
- Développer une simulation multi‑agents  
- Constituer un corpus multilingue d’interactions médiées par l’IA  

### Objectifs de Carrière  
- Renforcer l’expertise en modélisation cognitive  
- Construire un réseau Europe–Afrique  
- Publier des travaux interdisciplinaires à fort impact  

---

# PARTIE B2 — IMPACT  

## Impact sur le Chercheur  
- Expertise en modélisation cognitive  
- Visibilité internationale  
- Profil fortement interdisciplinaire  

## Impact sur l’Institution d’Accueil  
- Nouvelles collaborations  
- Innovation méthodologique  
- Recherche Europe–Afrique  

## Impact sur l’Europe  
- Contribution à l’éducation numérique  
- Cadres d’IA éthique  
- Innovation en sciences cognitives  

## Impact sur l’Afrique  
- Littératies numériques multilingues  
- Apprentissages inclusifs  
- Renforcement de capacités  

---

# PARTIE B3 — MISE EN ŒUVRE  

## Work Packages  

### WP1 — Fondements théoriques  
Philosophie du langage, sciences cognitives, théories de l’apprentissage, médiation par IA.  

### WP2 — Études empiriques et expérimentales  
Collecte de données, tâches multilingues, profils neurodivergents.  

### WP3 — Modélisation computationnelle  
Simulation multi‑agents des dynamiques langage–pensée.  

### WP4 — Diffusion & Impact  
Publications, ateliers, jeux de données ouverts, initiatives Afrique–Europe.  

---

## Calendrier (version texte)  
- M1–M6 : WP1  
- M4–M18 : WP2  
- M10–M24 : WP3  
- M1–M24 : WP4  

---

## Risques & Solutions  
- Recrutement faible → élargir les profils et réseaux  
- Complexité de la modélisation → prototypes successifs  
- Variabilité des données → triangulation méthodologique  

---

# Éthique & Science Ouverte  
- Respect du RGPD  
- Données anonymisées  
- Publications en accès libre  
- Respect des participants neurodivergents  

---

# Adéquation avec l’Institution d’Accueil & Plan de Formation  
L’institution d’accueil offre :  
- une expertise en sciences cognitives et IA  
- un environnement interdisciplinaire  
- un accès aux infrastructures de modélisation computationnelle  
- une intégration dans des réseaux européens  

Le plan de formation comprend :  
- des formations avancées en modélisation cognitive  
- des ateliers sur l’apprentissage médié par IA  
- des séminaires interdisciplinaires  
- des mobilités de recherche en Europe  

---

<hr style="margin-top:40px;">

<div style="text-align:center; font-size:0.85rem; opacity:0.85;">
  <a href="/about">About</a> •
  <a href="/theory">Theory</a> •
  <a href="/research">Research</a> •
  <a href="/msca">MSCA</a> •
  <a href="/ia4zep">IA_4_ZEP</a> •
  <a href="/faq">FAQ</a>
</div>
